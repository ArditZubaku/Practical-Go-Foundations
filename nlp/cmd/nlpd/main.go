...
var numTok = expvar.NewInt("tokenize.calls")
...
func main() {
	...
	http.HandleFunc("GET /stem/{word}", s.stemHandlerV2)
...
func (s *Server) stemHandler(w http.ResponseWriter, r *http.Request) {
	...
	handleWriteError(fmt.Fprintln(w, stem))
}

func (s *Server) stemHandlerV2(w http.ResponseWriter, r *http.Request) {
	word := r.PathValue("word")
	handleWriteError(fmt.Fprintln(w, stemmer.Stem(word)))
}
...
func (s *Server) tokenizeHandler(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if err := rdr.Close(); err != nil {
			slog.Error("Failed to close the http.MaxBytesReader", "err", err)
		}
	}()
...
func (s *Server) healthHandler(w http.ResponseWriter, _ *http.Request) {
	...
	handleWriteError(fmt.Fprintln(w, "OK"))
}
...
func handleWriteError(_ int, err error) {
	if err == nil {
		return
	}
